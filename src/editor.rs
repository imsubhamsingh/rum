...
impl Editor {
    ...
    fn move_cursor(&mut self, key: Key) {
        ...
        let height = size.height.saturating_sub(1) as usize; // saturating_sub() is a method on the usize type that subtracts 1 from the value unless it’s already 0, in which case it returns 0
        let width = size.width.saturating_sub(1) as usize;
        match key {
            Key::Up => y = y.saturating_sub(1), // saturating_sub() is a method on the usize type that subtracts 1 from the value unless it’s already 0, in which case it returns 0
            Key::Down => {
                if y < height {
